...
\begin{itemize}
  ...
  \item Level out the workload (heijunka). (Work like the tortoise, not the hare).
\end{itemize}
...
column as soon as they enter that stage of the process. It may seem too simplistic,
but we have to keep in mind Toyota's Way \textit{Use visual control so no problems are
hidden}. 

There is more than meets the eye though, because as we already stated before, Kanban is more
than visualization. Below we outline Kanban's core principles.
...
next when those conditions are met.

\subsubsection{Flow instead of batch}
CI
\subsubsection{Pull instead of push}
\subsubsection{Work in progress}
\subsubsection{Estimating, prioritization and scheduling}
None or velocity By sticky

\subsection{Relating to other methodologies}
\subsubsection{Open Allocation}
\subsubsection{Kanban with Scrum}
\subsubsection{Kanban instead of Scrum}
